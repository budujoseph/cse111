...
def pressure_gain_from_water_height(height):
    water_height_pressure = (998.2 * 9.80665 * height) / 1000
    return water_height_pressure
...
def pressure_loss_from_fittings(fluid_velocity, quantity_fittings):
    loss_from_fitting = (-0.04 * 998.2 * (fluid_velocity ** 2) * quantity_fittings) / 2000
    return loss_from_fitting

def reynolds_number(hydraulic_diameter, fluid_velocity):
    reys_number = (998.2 * hydraulic_diameter *fluid_velocity) / 0.0010016
    return reys_number

def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    constant = (0.1 + (50 / reynolds_number )) * (((larger_diameter / smaller_diameter) ** 4) - 1)
    loss_from_pipe_reduction = (-constant * 998.2 * (fluid_velocity ** 2)) / 2000
